...
// Update the existing getAllOrdersAction to use the correct admin endpoint
export async function getAllOrdersAction(queryString = "") {
  try {
    const token = await getAuthToken();
    if (!token) return { success: false, message: "Unauthorized" };
...
// Add order statistics action
export async function getOrderStatsAction() {
  try {
    const token = await getAuthToken();
    if (!token) return { success: false, message: "Unauthorized" };

    const response = await fetch(`${BACKEND_URL}/api/orders/admin/stats`, {
      headers: { Authorization: `Bearer ${token}` },
      cache: "no-store",
    });
...
// Update the existing updateOrderStatusAction to handle payment status too
export async function updateOrderStatusAction(id, payload) {
  try {
    const token = await getAuthToken();
    if (!token) return { success: false, message: "Unauthorized" };
